...
context DrawVertical do
  ...
  context 'second param' do
    it 'requires a number' do
      command = DrawVertical.new 1, 'x', 3, 'A'
      command.validate
      expect(command.error_message).to eq 'Second param is not an integer'
    end
    it 'is in the range 1..250' do
      command = DrawVertical.new 1, 251, 3, 'A'
      command.validate
  ...
  context 'third param' do
    it 'requires a number' do
      command = DrawVertical.new 1, 2, 'x', 'A'
      command.validate
      expect(command.error_message).to eq 'Third param is not an integer'
    end
    it 'is in the range 1..250' do
      command = DrawVertical.new 1, 2, 251, 'A'
      command.validate
  ...
  it 'draws the specified color in the given segment' do
    ...
    bitmap = DrawVertical.new(x, y1, y2, color).run bitmap
    (y1..y2).each do |y|
